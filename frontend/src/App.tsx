...
function App() {
  ...
  const update = <K extends keyof Project>(key: K, value: Project[K]) => {
    if (!project) return
    const scriptInputs = new Set<keyof Project>(['product_name', 'product_category', 'product_scale', 'product_dimensions', 'platform', 'duration', 'aspect_ratio', 'style', 'audience', 'selling_points', 'brief', 'script_template', 'narrative_pace', 'shot_count', 'hook_style', 'camera_style', 'lighting_style', 'emotion_curve', 'narration_density', 'product_exposure', 'transition_style', 'realism_level', 'negative_constraints'])
  ...
  const generate = async () => {
    ...
    try {
      const updated = await api.updateProject(project.id, {
        ...
        product_scale: project.product_scale, product_dimensions: project.product_dimensions,
      })
      setProject(updated)
  ...
  const editShot = (index: number, field: 'title' | 'visual' | 'action' | 'voiceover', value: string) => {
    if (!project?.creative_plan) return
    const shots = project.creative_plan.shots.map((shot, shotIndex) => shotIndex === index ? { ...shot, [field]: value } : shot)
    setProject({ ...project, creative_plan: { ...project.creative_plan, shots } })
  }

  const saveScript = async () => {
    if (!project?.creative_plan) return
    setBusy('save-script'); setMessage('')
    try {
      const updated = await api.saveScript(project.id, project.creative_plan)
      setProject(updated); setProjects(items => items.map(item => item.id === updated.id ? updated : item))
      setEditingScript(false); setMessage('脚本修改已保存，旁白已按各镜头时长自动校正。')
    } catch (error) { setMessage(error instanceof Error ? error.message : '脚本保存失败') }
    finally { setBusy('') }
  }
  ...
  const productionLocked = workflow?.status === 'running'
  ...
  return <div className="studio-shell">
    ...
    <main className={`studio-main panel-${activePanel}`}>
      ...
      <section className="workbench">
        <nav className="business-steps" aria-label="制作步骤"><span className={productReady ? 'done' : 'active'}><em>1</em>上传素材</span><i/><span className={!productReady ? '' : project.creative_plan ? 'done' : 'active'}><em>2</em>选择创意</span><i/><span className={project.creative_plan && workflow?.status !== 'completed' ? 'active' : workflow ? 'done' : ''}><em>3</em>生成镜头</span><i/><span className={workflow?.status === 'completed' ? 'done' : ''}><em>4</em>下载成片</span></nav>
        ...
        {activePanel === 'assets' ? <>
          ...
          <div className="asset-board">{assetCards.map(card => {
            const assets = project.assets.filter(item => item.kind === card.kind)
            const asset = assets.at(-1)
            return <label className={`asset-tile ${asset ? 'filled' : ''}`} key={card.kind}>
              <input type="file" accept="image/*" disabled={productionLocked} onChange={event => void upload(card.kind, event.target.files?.[0])}/>
              {assets.length ? <div className="asset-stack">{assets.map(item => <span key={item.id}><img src={item.url} alt={item.name}/><button type="button" title={`删除 ${item.name}`} onClick={event => { event.preventDefault(); event.stopPropagation(); void removeAsset(item) }}><Trash2/></button></span>)}</div> : <div className="upload-mark"><ImagePlus/><span>添加图片</span></div>}
              <div className="asset-meta"><span><b>{card.title}{assets.length > 1 ? ` · ${assets.length} 张` : ''}</b>{card.required && <em>必需</em>}<small>{asset?.name || card.hint}</small></span>{busy === `upload-${card.kind}` ? <LoaderCircle className="spin"/> : <Upload/>}</div>
            </label>
          })}</div>
          <div className="asset-tip"><Eye/><span><b>一致性提示</b>商品建议上传正面、侧面和使用状态；同一类素材可多次上传，系统将以最新图片为主要参考。</span></div>
        </> : <>
          <p className="section-intro">填写商品名称、尺寸和卖点即可。AI 会并行生成三个创意方向；高级导演参数可以保持默认。</p>
          ...
          <fieldset className="director-form" disabled={productionLocked}>
            <label className="wide">广告脚本模板<select value={project.script_template || 'story'} onChange={e => update('script_template', e.target.value)}><option value="story">真实故事推进</option><option value="problem-solution">问题—尝试—解决</option><option value="feature-proof">功能实证演示</option><option value="documentary">生活微纪录</option><option value="product-film">质感产品短片</option></select></label>
            <details className="director-console wide"><summary><span><b>高级导演设置</b><small>可选 · 默认值已经适合大多数商品广告</small></span><em>展开调整</em><ChevronDown/></summary><div className="control-matrix">
            ...
            <label>产品规格<select value={project.product_scale || 'tabletop'} onChange={e => update('product_scale', e.target.value)}><option value="handheld">手持小物</option><option value="tabletop">桌面产品</option><option value="floor">落地/大型产品</option><option value="custom">自定义尺寸</option></select></label>
            <label>真实尺寸<input value={project.product_dimensions || ''} onChange={e => update('product_dimensions', e.target.value)} placeholder="如：14.5 × 12 × 6.5 cm"/></label>
          ...
          {project.creative_plan && <div className={`script-preview ${editingScript ? 'editing' : ''}`}><div><small>LOCKED SCRIPT / 已锁定脚本</small><h3>{project.creative_plan.campaign_idea}</h3><p>{project.creative_plan.logline}</p></div>{project.creative_plan.shots.map((shot, index) => <article key={shot.id}><em>{String(index + 1).padStart(2, '0')}</em><span>{editingScript ? <><input value={shot.title} onChange={e => editShot(index, 'title', e.target.value)}/><textarea rows={2} value={shot.visual} onChange={e => editShot(index, 'visual', e.target.value)} aria-label={`镜头${index + 1}画面`}/><textarea rows={2} value={shot.action} onChange={e => editShot(index, 'action', e.target.value)} aria-label={`镜头${index + 1}动作`}/><textarea rows={2} value={shot.voiceover} onChange={e => editShot(index, 'voiceover', e.target.value)} aria-label={`镜头${index + 1}旁白`}/><small>建议旁白不超过 {Math.max(6, Math.floor(shot.duration * 3.8))} 个汉字</small></> : <><b>{shot.title}</b><p>{shot.visual}</p><p>{shot.action}</p><q>{shot.voiceover || '此镜头无旁白'}</q></>}</span><time>{shot.duration}s</time></article>)}<div className="script-actions"><button onClick={() => editingScript ? void saveScript() : setEditingScript(true)} disabled={!!busy}>{busy === 'save-script' ? <LoaderCircle className="spin"/> : <WandSparkles/>}{editingScript ? '保存脚本修改' : '自定义修改脚本'}</button><button onClick={() => void unlockScript()} disabled={!!busy}>{busy === 'unlock-script' ? <LoaderCircle className="spin"/> : <ChevronDown/>}返回候选脚本</button></div></div>}
        </>}
      ...
      <aside className="render-panel">
        <div className="render-heading"><span><Sparkles/></span><div><small>VOICE · MUSIC · OUTPUT</small><h2>输出设置</h2></div></div>
        ...
        {!providerAlert && !legacyDemoWorkflow && activeNode && ['failed', 'review_required'].includes(activeNode.status) && <QualityReviewCard node={activeNode} busy={busy} onRetry={() => void continueAfterReview('retry')} onAccept={() => void continueAfterReview('accept')}/>}
    ...
    {showSetup && <SetupModal current={setup} onClose={() => setup.ready && setShowSetup(false)} onSaved={status => { setSetup(status); setShowSetup(false); if (activeNode?.status === 'failed' && providerAlert?.type === 'auth') { setMessage('新密钥验证成功，正在从失败节点恢复。'); void continueAfterReview('retry') } }}/>} 
  </div>
}
...
function QualityReviewCard({ node, busy, onRetry, onAccept }: { node: WorkflowNode; busy: string; onRetry: () => void; onAccept: () => void }) {
  ...
  return <div className={`quality-review severity-${severity}`}>
    ...
    <div className="quality-actions"><button className="retry" disabled={!!busy} onClick={onRetry}>{busy === 'review-retry' ? <LoaderCircle className="spin"/> : <Sparkles/>}{retries > 0 ? '局部重做当前镜头' : '再次局部重做（需剩余预算）'}</button>{node.status === 'review_required' && <button className="accept" disabled={!!busy} onClick={onAccept}><Check/>接受风险并继续</button>}</div>
  </div>
}
...
function PreviewPanel({ project, workflow, output, activeNode, completedShots, totalShots, onSelectAttempt }: { project: Project; workflow: WorkflowRun | null; output: string; activeNode?: WorkflowNode; completedShots: number; totalShots: number; onSelectAttempt: (nodeId: string, attempt: number) => void }) {
  ...
  return <section className="preview-stage">
    ...
    {shotTimeline.length > 0 && <div className="shot-stream"><header><span><b>镜头流</b><small>镜头完成即进入时间线，视觉质检在后台自动处理</small></span><em>{completedShots}/{shotTimeline.length}</em></header><div>{shotTimeline.map(({ shot, generation, review, attempts, activeAttempt }) => {
      ...
      const preview = browserMediaUrl(attempts.find(item => item.number === activeAttempt)?.outputs.video_url || attempts.at(-1)?.outputs.video_url || attempts.at(-1)?.outputs.video_path)
      return <article key={shot.id} className={`${generation?.status || 'blocked'}`}>{preview && <video src={preview} controls preload="metadata"/>}<span className="shot-index">{String(shot.index).padStart(2, '0')}</span><div><b>{shot.title}</b><small>{shot.duration}s · {generation?.status === 'running' ? '生成中' : generation?.status === 'completed' ? '镜头可用' : generation?.status === 'failed' ? '生成失败' : '等待队列'}</small>{review?.status === 'running' && <p><LoaderCircle className="spin"/>正在后台检查</p>}{review?.status === 'completed' && <p><Check/>{optimized ? '已自动优化' : '已完成自动检查'}</p>}{attempts.length > 1 && <nav>{attempts.map(item => <button key={item.number} className={item.number === activeAttempt ? 'selected' : ''} onClick={() => onSelectAttempt(generation!.id, item.number)}>候选 {item.number}</button>)}</nav>}</div><i>{generation?.status === 'completed' ? <Check/> : generation?.status === 'running' ? <LoaderCircle className="spin"/> : <Clock3/>}</i></article>
    })}</div></div>}
  </section>
}
