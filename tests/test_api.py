...
def test_locked_script_update_is_json_body_and_limits_narration(monkeypatch) -> None:
    monkeypatch.delenv("DIRECTOR_API_BASE", raising=False)
    monkeypatch.delenv("DIRECTOR_MODEL", raising=False)
    created = client.post("/api/projects", json={
        "name": "脚本编辑回归",
    ...
    try:
        client.patch(f"/api/projects/{project_id}", json={
            "product_scale": "custom",
            "product_dimensions": "14.5 × 12 × 6.5 cm",
        })
        planned = client.post(f"/api/projects/{project_id}/plan")
        plan = planned.json()["creative_plan"]
        plan["shots"][0]["voiceover"] = "这是一段明显超过当前五秒镜头自然语速承载能力并且必须自动缩短的旁白文字。"
        saved = client.put(f"/api/projects/{project_id}/script", json=plan)
        assert saved.status_code == 200, saved.text
        assert len(saved.json()["creative_plan"]["shots"][0]["voiceover"]) <= 20
        assert saved.json()["product_dimensions"] == "14.5 × 12 × 6.5 cm"
    finally:
        client.delete(f"/api/projects/{project_id}")


def test_project_mutations_are_blocked_while_production_is_claimed() -> None:
    import backend.app.main as main_module

    created = client.post(
        "/api/projects",
        json={"name": "production-lock-test", "product_name": "test-toy"},
    )
    project_id = created.json()["id"]
    assert main_module._claim_production(project_id) is True
    try:
        ...
        main_module._release_production(project_id)
        client.delete(f"/api/projects/{project_id}")
